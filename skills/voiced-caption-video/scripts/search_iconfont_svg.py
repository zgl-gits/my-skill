#!/usr/bin/env python3
"""Search iconfont.cn and save SVG icons for video/animation assets."""
...
def search_iconfont(query: str, limit: int, icon_type: str, ctoken: str) -> list[dict]:
    ...
    body = (
        ...
        f"&sType=&fromCollection=-1&fills=&t={int(time.time() * 1000)}&ctoken={quote_plus(ctoken)}"
    )
    headers = {
...
def main() -> None:
    ...
    parser.add_argument("--limit", type=int, default=12, help="Number of search results to inspect/save.")
    parser.add_argument(
        "--type",
        default="",
